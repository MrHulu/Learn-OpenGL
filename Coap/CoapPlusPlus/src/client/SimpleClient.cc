...
SimpleClient::SimpleClient(Context& context) : ctx(context)
{
...
{
  uint32_t wait_ms = timeout_ms > 0 ? timeout_ms : 
      timeout_ms == 0 ? COAP_IO_WAIT : COAP_IO_NO_WAIT;
  {
    std::lock_guard<std::mutex> lock(m_mutex);
    m_flag = true;
  }
  ...
  while (true) { 
    ...
    if (result < 0)  
      ...
      std::lock_guard<std::mutex> lock(m_mutex);
      wait_ms = timeout_ms > 0 ? timeout_ms : 
        timeout_ms == 0 ? COAP_IO_WAIT : COAP_IO_NO_WAIT;;
    }
  }
}
...
void SimpleClient::observe(const std::string &urlPath, CallbackFunc callback)
{
  ...
  */ 
  ...
  // coap_context_set_block_mode(ctx.m_ctx, COAP_BLOCK_USE_LIBCOAP);
...
{
  coap_register_response_handler(ctx.m_ctx,  defaultResponse);
}
