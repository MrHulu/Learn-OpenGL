...
void tst_Communication::constom_resource_test()
{
    ...
    try {
        auto resource = std::make_shared<Tst_TextResource>(server->context(), textUrl);
...
{
    qDebug() << "delete_test server";
    delete server;
    qDebug() << "delete_test client";
    delete client;
    qDebug() << "delete_test end";
}
